...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            month_frame, text="Mese:", background=common_text_format.get("background")
        ).pack()

        self.month_var = tk.StringVar()
        month_spinbox = ttk.Spinbox(
            month_frame, from_=1, to=12, textvariable=self.month_var, width=5
        )
        month_spinbox.pack()
        ...
        ttk.Label(
            day_frame, text="Giorno:", background=common_text_format.get("background")
        ).pack()

        self.day_var = tk.StringVar()
        day_spinbox = ttk.Spinbox(
            day_frame, from_=1, to=31, textvariable=self.day_var, width=5
        )
        day_spinbox.pack()
        ...
        find_button = ttk.Button(
            button_frame, text="Calcola il segno Zodiacale", command=self.find_zodiac
        )
        find_button.pack()
        ...
        self.image_label = ttk.Label(
            image_frame, background=common_text_format.get("background")
        )
    ...
    def find_zodiac(self):
        ...
        try:
            ...
            self.result_label.config(
                text=f"Il tuo segno zodiacale è: {zodiac_sign}", foreground="green"
            )
            self.load_zodiac_image(zodiac_sign)
        except ValueError:
            self.result_label.config(
                text="Please enter valid numbers.", foreground="red"
            )
            self.image_label.config(image="")
            self.current_image = None
            self.result_label.config(
